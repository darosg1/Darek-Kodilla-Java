...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        OptionalDouble avrDaysInProgressTask = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> t.getCreated())
        ...
        //Then
        Assert.assertEquals(14.000, avrDaysInProgressTask.getAsDouble(), 0.001);
    }
    @Test
    public void testAddTaskListAverageWorkingOnTask2() {
        ...
        inProgressTasks.add(new TaskList("In progress"));
        inProgressTasks.add(new TaskList("To do"));
        double result1 = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> t.getCreated())
                .map(day -> ChronoUnit.DAYS.between(day, LocalDate.now()))
        ...
        double result2=project.getTaskLists().stream()
                ...
                .count();
        //Then
        Assert.assertEquals(14.000, result1/result2, 0.001);
    }
}
